# debug_and_fix_dataset_fixed.py
import pandas as pd
import numpy as np
...
def debug_dataset():
    ...
    print(f"Class balance: {df['is_linker'].value_counts().to_dict()}\n")
...
def create_working_dataset():
    """Create a working dataset with valid SMILES and features"""
    print("\n=== 🛠️ CREATING WORKING DATASET ===\n")
    
    # Load the original dataset
    df = pd.read_csv('linkermind_final_balanced_dataset.csv')
    
    # Filter valid SMILES and calculate features
    valid_data = []
    ...
    for idx, row in df.iterrows():
        ...
        if mol is not None:
            # Calculate basic properties (using available descriptors)
            ...
            # Calculate Fraction Csp3 manually if needed
            try:
                ...
                if total_carbons > 0:
                    sp3_carbons = sum(1 for atom in mol.GetAtoms() if atom.GetAtomicNum() == 6 and atom.GetHybridization().name == 'SP3')
                    fraction_csp3 = sp3_carbons / total_carbons
                else:
                    fraction_csp3 = 0.0
    ...
    print(working_df['is_linker'].value_counts().to_dict())
...
if __name__ == "__main__":
    ...
    print("👉 Next command: python simple_training_pipeline.py")
